...
client.send(Orders.HTD)
	.then((resp) => {
		...
		let distPath = global.entity ? client.storageLocation+"HTD_"+client.bankShortName+"_EBICS_"+global.entity+"_"+splitDate[0].replace("-","").replace("-","")+".xml" : client.storageLocation+"HTD_"+client.bankShortName+"_EBICS_"+splitDate[0].replace("-","").replace("-","")+".xml";
