...
client.send(Orders.HKD)
	.then((resp) => {
		...
		let distPath = global.entity ? client.storageLocation+"HKD_"+client.bankShortName+"_EBICS_"+global.entity+"_"+splitDate[0].replace("-","").replace("-","")+".xml" : client.storageLocation+"HKD_"+client.bankShortName+"_EBICS_"+splitDate[0].replace("-","").replace("-","")+".xml";
