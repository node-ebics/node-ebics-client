...
client.send(Orders.DKI(null, null)) // startDate 'YYYY-MM-DD', endDate 'YYYY-MM-DD'
	.then((resp) => {
		...
		let distPath = global.entity ? client.storageLocation+"DKI_"+client.bankShortName+"_EBICS_"+global.entity+"_"+splitDate[0].replace("-","").replace("-","")+".txt" : client.storageLocation+"DKI_"+client.bankShortName+"_EBICS_"+splitDate[0].replace("-","").replace("-","")+".txt";
